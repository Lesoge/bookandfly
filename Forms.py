...
class PaymentForm(Form):
    first_name = StringField('First Name', [validators.InputRequired(), validators.Length(1, 15, message='Input too long or short')])
    last_name = StringField('Last Name', [validators.InputRequired(), validators.Length(1, 15, message='Input too long or short')])
    street = StringField('Street', [validators.InputRequired(), validators.Length(1, 20, message='Input too long or short')])
    street_number = StringField('Street Number', [validators.InputRequired(), validators.Regexp('\d')])
    town = StringField('Town', [validators.InputRequired(), validators.Length(1, 20, message='Input too long or short')])
    ...
    name_on_card = StringField('Credit Card Holder', [validators.InputRequired(), validators.Length(1, 31, message='Input too long or short')])
    # todo adjust to needed input
    expiry_date = StringField('Expiry Date', [validators.InputRequired(),
                                              validators.Regexp('\d{2}[/]\d{2}$', flags=0, message='Has to be the form mm/yy')])
...
class SignUpForm(Form):
    ...
    username = StringField('Username', [validators.InputRequired(), validators.Regexp('[a-z]*[A-Z]*[0-9]*\s*', flags=0, message='Username can not contain special characters')])
